...
| App | `obsidian-eclipse` pins `2f8509c` for `flutter_seekserve` and `flutter_seekserve_ui` since app commit `ed706005` (2026-09-25, not yet published: publish this branch first; the app's pub cache got the commit from this clone), `847c1be` since `ad596493` (2026-09-23; before that `3bc9aa0`, `782f2ee` and `38cf24a`), and ships an Android `libseekserve.so` built from it (arm64-v8a; the unused armeabi-v7a one is still from `847c1be`) (committed in `android/app/src/main/jniLibs/arm64-v8a/`, sentinel `.seekserve_build_commit`). Since app commit `9cd94f4d` the APK holds arm64-v8a only, the only ABI with Tor (app BUG-19); the armeabi-v7a library still in the app repository is unused (removal in the app's WORK-10) |
